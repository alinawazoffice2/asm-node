        ...
        for (let i = 0; i < tree.length; i++) {
            ...
            if(node.name == startup_method_name){
                ...
                for (let j = 0; j < code_blocks.length; j++) {
                    ...
                    if(block.command == 'stf'){
                        const fs = require('fs');
                        fs.writeFile(block.filename, me.stacker.join(','), function(err) {
                            if(err) {
                    ...
                    if(block.command == 'stfp'){
                        const fs = require('fs');
                        fs.readFile(block.filename, "utf8", function(err, data) {
                            fs.writeFile(block.filename, data + ',' + me.stacker.join(','), function(err) {
                                if(err) {
                                    return console.log(err);
                                }
                            });
                    ...
                    if(block.command == 'pts'){
                        if(me.isRegister(block.from)){
                            me.stacker.push(me.getRegister(block.from));
                        }else{
                            me.stacker.push(block.from);
                        }
                    } 

                    if(block.command == 'pfs'){
                        if(me.stacker.length == 0){
                            console.log('WARN: stacker is empty.');
                        }else{
                            if(me.isRegister(block.to)){
                                me.setRegister(block.to, me.stacker.pop());
                            }
                        }
                    } 
